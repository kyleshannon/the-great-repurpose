...
The Daily Signal section reads public JSON at runtime from GitHub Raw first and
jsDelivr second, with RSS and bundled fallback data kept in sync from the same
local repo. To publish a new daily briefing, write the Signal JSON files,
regenerate the feed and bundled fallback data, validate, run a production build,
commit, push from this canonical clone, then purge jsDelivr.

Canonical publishing clone:

```text
/Users/kyleshannonm5/Documents/Codex/projects/the-great-repurpose/website/
```
...
3. `public/signals.xml` - regenerated by `node scripts/build-signals-rss.mjs`.
4. `src/data/generatedSignals.ts` - regenerated by `node scripts/build-signals-rss.mjs` as the bundled fallback source.
...
9. Run `node scripts/build-signals-rss.mjs` so `public/signals.xml` and
   `src/data/generatedSignals.ts` include today's issue.
10. Run `npm run build`. Do not commit if the production build fails.
11. Commit all four publish files in one commit:
   `public/signals/<slug>.json`, `public/signals/index.json`,
   `public/signals.xml`, and `src/data/generatedSignals.ts`. Suggested message:
   `signal: <date> — <title>`.
12. Push to `main` on `kyleshannon/the-great-repurpose` from the canonical
   local clone above. Do not publish the daily Signal from a second clone or
   through a separate GitHub file-write path.
13. **Purge the jsDelivr cache** so the live site picks up the change
...
14. Verify jsDelivr serially with short timeouts. Do not run these checks in
    parallel:
    - `https://cdn.jsdelivr.net/gh/kyleshannon/the-great-repurpose@main/public/signals/index.json`
      must have today's slug as the first entry.
    - `https://cdn.jsdelivr.net/gh/kyleshannon/the-great-repurpose@main/public/signals/<slug>.json`
      must return the matching slug and title.
15. Verify both the CDN and the live rendered site. CDN verification alone is
    not enough.
...
> purged. This jsDelivr purge is the known-good mechanism for making the daily
> Signal visible without waiting for Lovable to republish.
...
- `public/signals.xml` contains today's slug.
- `src/data/generatedSignals.ts` contains today's slug in both the bundled
  index and bundled detail map.
